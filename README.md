...
```json
{
  ...
  "gas_endpoint_profiles": {
    "normal": {
      "test": "https://script.google.com/macros/s/TEST_DEPLOYMENT_ID/exec",
      "prod": "https://script.google.com/macros/s/PROD_DEPLOYMENT_ID/exec"
    },
    "infotop": {
      "test": "https://script.google.com/macros/s/INFOTOP_TEST_DEPLOYMENT_ID/exec",
      "prod": "https://script.google.com/macros/s/INFOTOP_PROD_DEPLOYMENT_ID/exec"
    }
  },
  "sales_channels": {
    "normal": {
      "purchase_url": "/checkout/checkout.html",
      "hide_demo": false,
      "checkout_redirect": false
    },
    "infotop": {
      "purchase_url": "https://www.infotop.jp/order.php?iid=XXXXXX",
      "hide_demo": true,
...
`sales_channel`は販売導線の切り替えに使用します。`normal`ではサイト内の購入フォームを使用し、`infotop`では購入リンクを`purchase_url`へ差し替え、デモ版の導線を非表示にします。

`gas_profile`は`gas_endpoint_profiles`内のどのGAS URLセットを使うかを指定します。インフォトップ用のGASやスプレッドシートを分ける場合は、`infotop`側のURLを差し替えてください。
...
### developとmasterへの一括反映

現在の差分を`develop`へコミットし、`master`へマージして両ブランチをプッシュする場合は、次のコマンドを実行します。

```bash
node scripts/publish-all.mjs
```

変更対象と自動生成されるコミットメッセージだけを確認する場合は、`--dry-run`を指定します。
